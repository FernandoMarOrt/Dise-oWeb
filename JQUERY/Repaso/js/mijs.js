...
clic sobre el icono -, se debe realizar el efecto contrario y el - cambiarse por un +. */
    ...
    $(document).ready(function () {
        $('#menu-toggle>li').has('ul').on('click', function () {
            ...
            if (ulElement.is(':visible')) {     // Si el UL actualmente está abierto
                ulElement.slideUp('slow');
                spanElement.text('+');
            } else {
                $('#menu-toggle>li>ul').slideUp('slow');       // Cierra todos los UL 
                ulElement.css({ left: '-50vw', display: 'block' }).animate({ left: 0 }, 'slow');//efecto y abrir
                spanElement.text('-'); //cambia los textos de los span
            }
        });
    });
...
$(document).ready(function () {
    ...
    $("textarea").on("input", function () {
        ...
        $("#info-caracteres").text("Dispone de " + caracterTexto + " caracteres de " + caracterMax);
    });
